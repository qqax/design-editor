...
class Events extends Base {
  ...
  private onMouseDown = (e: any) => {
    ...
    if (e.e.button === 2) {
      // 2 is right click in standard MouseEvent (changed from 3)
      this.state.setContextMenuRequest({
        left: (e.e as MouseEvent).offsetX,
        top: (e.e as MouseEvent).offsetY,
        target: e.target,
      });
    } else {
  ...
  handleSelection = (target: any) => {
    if (target) {
      ...
      if (
        initialSelection &&
        !isNotMultipleSelection &&
        initialSelection._objects
      ) {
        const filteredObjects = (
          initialSelection._objects as FabricObject[]
        ).filter((object) => {
