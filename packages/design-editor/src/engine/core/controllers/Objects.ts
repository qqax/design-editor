...
import type { StaticText } from '../../objects/StaticText';
...
class Objects extends Base {
  ...
  public add = async (item: Partial<ILayer>) => {
    const { canvas } = this;
    const { options } = this.editor.frame;
  ...
  public update = (options: Partial<ILayerOptions>, id?: string) => {
    ...
    const { canvas } = this;
    if (refObject) {
      const { isEditing } = refObject as any;
      const hasSelection =
        isEditing &&
        (refObject as any).selectionStart !== (refObject as any).selectionEnd;
      ...
      for (const property in options) {
        if (
          ...
          refObject.type === LayerType.ACTIVE_SELECTION &&
          (refObject as any)._objects
        ) {
          // @ts-ignore
  ...
  public reset = () => {
    const { background } = this.editor.frame;
  ...
  private async duplicate(object: FabricObject): Promise<FabricObject[]> {
    if (object instanceof Group && object.type !== LayerType.STATIC_VECTOR) {
      const objects = object.getObjects();

      const duplicates = (
        await Promise.all(objects.map(async (child) => this.duplicate(child)))
      ).flat();
    ...
    if (this.config.clipToFrame) {
      clone.clipPath = this.editor.frame.frame as unknown as FabricObject;
    }
  ...
  public sendBackwards = (id?: string) => {
    ...
    const backgroundImage = objects.find(
      (o) => o.type === LayerType.BACKGROUND_IMAGE
    );
  ...
  public async unsetBackgroundImage(): Promise<StaticImage | null> {
    return new Promise(async (resolve) => {
      const objects = this.canvas.getObjects();
      const currentBackgroundImage = objects.find(
        (o) => o.type === LayerType.BACKGROUND_IMAGE
      );
      let nextImage: StaticImage;
      if (currentBackgroundImage) {
        const currentBackgroundImageJSON = currentBackgroundImage.toObject(
            this.config.propertiesToInclude
        ) as Record<string, any>;

        delete currentBackgroundImageJSON.clipPath;

        const nextImageElement = await loadImageFromURL(
            currentBackgroundImageJSON.src
        );

        nextImage = new StaticImage(nextImageElement, {
  ...
  public lock = (id?: string) => {
    let refObject = this.canvas.getActiveObject() as
      FabricObject | ActiveSelection;
    if (id) {
      refObject = this.findOneById(id);
    }
    if (refObject) {
    ...
    if (refObject && refObject.type === LayerType.STATIC_TEXT) {
      if (refObject.isEditing) {
        refObject.hiddenTextarea!.value =
          refObject.hiddenTextarea!.value.toUpperCase();
        refObject.updateFromTextArea();
        this.canvas.requestRenderAll();
        return;
      }

      const { text } = refObject;
  ...
  public toLowerCase(id?: string) {
    ...
    if (refObject && refObject.type === LayerType.STATIC_TEXT) {
      if (refObject.isEditing) {
        refObject.hiddenTextarea!.value =
          refObject.hiddenTextarea!.value.toLowerCase();
        refObject.updateFromTextArea();
        this.canvas.requestRenderAll();
        return;
      }
