...
export function angleToPoint(angle: number, sx: number, sy: number) {
  ...
  const a = sy;
  const b = a + sx;
  const c = b + sy; // 3 first corners
  const p = (sx + sy) * 2; // perimeter
  const rp = p * 0.00277; // ratio between perimeter & 360
  const pp = Math.round((angle * rp + (sy >> 1)) % p); // angle position on perimeter
